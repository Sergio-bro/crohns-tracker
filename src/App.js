...
const LANGS = {
  en: {
    appTitle: "Crohn's Care Tracker",
    appSub: "Health monitoring · Taiwan",
    ...
    type: "Type", time: "Time", prep: "Prep Required",
  ...
  ru: {
    ...
    appSub: "Мониторинг здоровья · Тайвань",
    ...
    type: "Тип", time: "Время", prep: "Подготовка",
    medication: "Лекарство", frequency: "Частота",
  ...
  zh: {
    ...
    type: "類型", time: "時間", prep: "準備事項",
    medication: "藥物", frequency: "頻率",
...
function GrowthChart({ data, t }) {
  if (data.length < 2) return <div style={{ color:"#94a3b8", fontSize:13, textAlign:"center", padding:20 }}>{t.noData} (need 2+ entries)</div>;
  const sorted = [...data].sort((a,b)=>a.date.localeCompare(b.date));
  const weights = sorted.map(d=>d.weight).filter(Boolean);
  const heights = sorted.map(d=>d.height).filter(Boolean);
    ...
    <div style={{ fontFamily:"system-ui,sans-serif", background:"#f1f5f9", minHeight:"100vh" }}>
      ...
      <div style={{ maxWidth:680, margin:"0 auto", padding:16 }}>
        ...
        {tab===1 && <>
          ...
          {[...data.treatments].sort((a,b)=>a.date.localeCompare(b.date)).map((tr,i)=>(
            <Card key={tr.id}>
        ...
        {tab===2 && <>
          ...
          {[...data.appointments].sort((a,b)=>a.date.localeCompare(b.date)).map(ap=>(
            <Card key={ap.id}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:10 }}>
                <div style={{ flex:1 }}>
        ...
        {tab===4 && <>
          ...
          <Card>
            ...
            <GrowthChart data={data.growth} t={t}/>
          </Card>
          {[...data.growth].sort((a,b)=>b.date.localeCompare(a.date)).map(g=>{
        ...
        {tab===5 && <>
          ...
          {data.symptoms.length===0 && <Card><div style={{ color:"#94a3b8", textAlign:"center", padding:20 }}>💡 Log symptoms during flare-ups or when something feels different.</div></Card>}
        ...
        {tab===6 && <>
          ...
          {[...data.notes].sort((a,b)=>b.date.localeCompare(a.date)).map(n=>{
